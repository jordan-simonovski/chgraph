...
| 2 | Vector | Brute-force `cosineDistance` over `Array(Float32)` (HNSW `vector_similarity` is compiled out of chdb — VERIFIED, owned by **chdb-reference**) | RESOLVED (ADR-0004): model = fastembed `bge-small-en-v1.5` (384-dim, ONNX, optional dep); embed-text = subtokens(name)+first-para docstring; latency VERIFIED 36ms over 46k×384 → no ANN needed; semantic goldens MRR 0.000 (vec off) → 1.000 (vec on) on flask. |
